...
async function renderRelatedActivities(activityId, destinationId, dataStore) {
  ...
  randomActivities.forEach((activity) => {
    ...
    activityElement.addEventListener("click", function () {
      ...
      const relatedActivity = findActivityByIdInDataStore(
        dataStore,
        relatedActivityId
      );
...
async function showActivityModal(activityId, dataStore) {
  ...
  let foundObject = clickedActivity._place._favorite_of_user_of_place.find(
...
window.onload = async () => {
  ...
  Wized.request.awaitAllPageLoad(async () => {
    ...
    if (destinationId) {
      for (let guide of dataStore._guide_of_trips) {
    ...
    function flyToAndSetActive(place) {
      ...
      const newActiveMarker = document.querySelector(
        `[marker_activity_id="${place.place_id}"]`
      );
